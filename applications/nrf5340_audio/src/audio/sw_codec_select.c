...
int sw_codec_encode(void *pcm_data, size_t pcm_size, uint8_t **encoded_data, size_t *encoded_size)
{
	...
	case SW_CODEC_OPUS: {
		...
		case SW_CODEC_MONO: {
			nbBytes = opus_encode(encoder_left_channel,
		...
		case SW_CODEC_STEREO: {
			...
			ret = opus_encode(encoder_right_channel,
					  ...
					  m_encoded_data + nbBytes,
					  sizeof(m_encoded_data) - nbBytes);
			if (ret <= 0) {
		    ...
		    void **decoded_data, size_t *decoded_size)
{
	...
	case SW_CODEC_OPUS: {
		...
		case SW_CODEC_MONO: {
			...
			/* 16 bits per samples, maybe there exist a macro/define i can use here instead? */
			pcm_size_session *= 2;
		...
		case SW_CODEC_STEREO: {
			/* Remove this part? As each headset should only decode mono? */
			/* int16_t pcm_data_mono_right[PCM_NUM_BYTES_MONO] = { 0 };
			pcm_size_session = opus_decode(decoder, encoded_data, encoded_size / 2,
						       pcm_data_mono, OPUS_MAX_FRAME_SIZE, 0);
			if (pcm_size_session < 0) {
				LOG_ERR("Failed to decode (err: %d)", pcm_size_session);
				return pcm_size_session;
			}
			LOG_INF("Left_pcm: %d", pcm_size_session);

			pcm_size_session = opus_decode(decoder, (encoded_data + (encoded_size / 2)),
						       encoded_size / 2, pcm_data_mono_right,
						       OPUS_MAX_FRAME_SIZE, 0);
			if (pcm_size_session < 0) {
				LOG_ERR("Failed to decode (err: %d)", pcm_size_session);
				return pcm_size_session;
			}
			LOG_INF("Right_pcm: %d", pcm_size_session);
...
int sw_codec_init(struct sw_codec_config sw_codec_cfg)
{
	...
	case SW_CODEC_OPUS: {
		...
		if (sw_codec_cfg.encoder.enabled) {
			...
			if (err < 0) {
				LOG_ERR("Failed to create the OPUS Encoder (Left channel) (err: %d)",
					err);
				return err;
			}
			...
			if (err) {
				LOG_ERR("Failed to configure OPUS encoder (Left channel) (err: %d)",
					err);
				return err;
			}
			...
			encoder_right_channel = opus_encoder_create(
				CONFIG_OPUS_SAMPLE_RATE, CONFIG_OPUS_CHANNELS, APPLICATION, &err);
			if (err < 0) {
				LOG_ERR("Failed to create the OPUS Encoder (Right channel) (err: %d)",
					err);
				return err;
			}
			...
			if (err) {
				LOG_ERR("Failed to configure OPUS encoder (Right channel) (err: %d)",
					err);
				return err;
			}
		}
